...
Dette skriptet ser på hvordan *sammenhengen* mellom hvert partis oppslutning og de tre levekårsvariablene har endret seg fra valg til valg. Bruker alle fem valgårene prosjektet har data for: lokalvalgene 2015, 2019 og 2023, og stortingsvalgene 2021 og 2025. I motsetning til resten av prosjektet er **Sentrum ikke holdt utenfor** her; alle bydeler er med. Figurene fargelegger partiene etter plassering på venstre-høyre-aksen (`RColorBrewer::brewer.pal(9, "RdBu")`), med Rødt i mørkest rødt og Frp i mørkest blått, avtagende metning mot midten (Sp/KrF).

Metode:

1. For hvert parti, valgår og variabel: en enkel lineær regresjon `Oppslutning ~ variabel` på kretsnivå, som gir et stigningstall, standardfeil, p-verdi og forklart varians (R²). Fullt resultat i `data/regresjonsresultater_partiutvikling.csv`, figur i `figurer/utvikling_regresjonskoeffisienter.png`.
2. En formell test av om stigningstallet har endret seg signifikant over de fem valgene: `Oppslutning ~ variabel × valgår` med valgår som kontinuerlig variabel, per parti og variabel. Resultat i `data/regresjonsresultater_interaksjon.csv`.
3. **Kontrollert (multivariat) regresjon**, siden de tre levekårsvariablene korrelerer sterkt med hverandre på delbydelsnivå (innvandrerandel↔utdanning: r = −0,79, innvandrerandel↔inntekt: r = −0,58, inntekt↔utdanning: r = 0,60) – en enkel regresjon kan derfor blande sammen effektene. Her kjøres `Oppslutning ~ inntekt + innvandrerandel + utdanning` samtidig, som gir hver variabels partielle (kontrollerte) sammenheng. Resultat i `data/regresjonsresultater_multivariat.csv`, sammenligning enkel/kontrollert i `data/regresjonsresultater_sammenligning.csv` og `figurer/utvikling_regresjon_kontrollert_vs_enkel.png`.
4. **Multikollinearitet og samspill.** Den additive kontrollen i (3) antar at hver variabels effekt er uavhengig av de to andre – rimelig å stille spørsmål ved når variablene i praksis henger sammen som del av det man gjerne omtaler som "klasse". Multikollineariteten er sjekket med VIF (`data/regresjonsresultater_vif.csv`) – moderat (1,6–3,9), ikke alvorlig. Deretter er det testet for reelle samspillsledd (`Oppslutning ~ innvandrerandel × inntekt` og `× utdanning`, standardiserte variabler) per parti og år, i `data/regresjonsresultater_samspill.csv` og `figurer/samspill_levekarsvariabler.png`.

**Hovedfunn:**

- Arbeiderpartiets sammenheng med innvandrerandel og utdanningsnivå er klart svekket gjennom perioden (interaksjonsledd med desidert lavest p-verdi av alle), mens sammenhengen med inntekt består best. Høyres sammenheng med inntekt er òg signifikant svekket, men fortsatt den sterkeste enkeltsammenhengen i datasettet.
- **Hvor mye av dette er egentlig innvandrerandel, kontrollert mot de andre variablene?** For Ap er svaret ganske ulikt for de to variablene: 80–100 % av den tilsynelatende sammenhengen med innvandrerandel forsvinner når inntekt og utdanning holdes fast – den er altså stort sett en proxy for lav inntekt/lavt utdanningsnivå, ikke en selvstendig "innvandrer"-effekt. Sammenhengen med utdanningsnivå er derimot mer robust: bare 17–37 % forsvinner ved kontroll, dvs. at Aps svekkede oppslutning i lavutdanningskretser i stor grad står seg som en selvstendig sammenheng.
- **SV i 2025 er det tydeligste eksempelet på ren konfundering**: den tilsynelatende negative sammenhengen med utdanningsnivå i 2025 (−0,41) krymper med 91 % når innvandrerandel og inntekt kontrolleres for (til −0,04, praktisk talt null) – dvs. at SVs "svakere i høyt utdannede kretser"-mønster i 2025 nesten utelukkende skyldes at partiet gjør det sterkt i kretser med høy innvandrerandel, som også har lavere utdanningsnivå, ikke utdanning i seg selv. I de fire foregående valgene er bildet motsatt: SVs kontrollerte (rensede) sammenheng med utdanning er faktisk *positivt*, og delvis maskert av den negative korrelasjonen mellom innvandrerandel og utdanning – 2025 skiller seg dermed ut som et brudd med det mønsteret, ikke en forlengelse av det.
- Rødt og SV har begge fått en gradvis sterkere sammenheng med innvandrerandel over tid, og denne står seg langt på vei også etter kontroll. For FRP, KrF, MDG og Venstre er endringene enten svakere eller ikke statistisk sikre i dette datagrunnlaget.

**Samspillsfunn (utover den additive kontrollen over):** Samspillet mellom innvandrerandel og inntekt er reelt, systematisk og har blitt sterkere gjennom perioden – og det gjelder ikke bare SV, men speiler seg symmetrisk over hele venstre-høyre-aksen. For venstresidepartiene (Ap, Rødt, SV, MDG) er samspillsleddet negativt og økende i styrke: inntektssammenhengen (og dermed oppslutningen) er sterkere i kretser med høy innvandrerandel enn i kretser med lav. For høyresidepartiene, særlig Frp og Høyre, er samspillsleddet positivt og økende i samme periode – motsatt utslag i samme retning. Med andre ord: det er ikke fattigdom og innvandrerandel hver for seg som driver polariseringen, det er *kombinasjonen* av lav inntekt og høy innvandrerandel som har fått en stadig sterkere, selvstendig effekt på tvers av hele det politiske spekteret. Samspillet mellom innvandrerandel og utdanning er langt svakere og mer ustabilt fra år til år, uten noe tilsvarende konsistent mønster.
